...
2. 普通和技能 manager 模式的维度键均来自推送执行、活跃owner、Span、点击四条事实路径的 UNION；名单限制每条路径的范围。普通模式去重为机构+经理，技能模式去重为机构+经理+技能，无事实经理不占分页 total。
3. 对维度键 COUNT 得到 total，再稳定 ORDER BY 分行/支行/经理/技能并 LIMIT/OFFSET 取页。
...
相比无分页，分页增加固定的维度键计数、取页及校验查询，不随经理人数循环查库。技能键发现仍需扫描符合条件的事实范围，并不等于廉价的主键分页；查询计划、UNION去重、分片路由和深OFFSET性能需在真实 TDSQL 上验证。普通指标聚合仅针对本页键，未引入缓存或新表。
