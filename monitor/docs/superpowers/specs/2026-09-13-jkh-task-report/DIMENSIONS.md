...
- `skill_count` 沿用原统计过滤，在单技能类型行是 0 或 1。2026-09-16 起推送侧统一限定「未删除 job + 至少一个统计技能」，删除 job 或统计开关全关时该 job 的执行、方案、技能与活跃一起消失，不再出现「保留执行/方案数但技能数为 0」的行。
...
2026-09-16 修正：汇总以期间事实实际出现的机构/经理为骨架，每个有效维度对象补齐三类任务。名单仅限定范围、提供归属/元数据及原口径权限人数，无事实的维度不输出；有事实但指标为零的维度仍保留。指定 `task_type` 时骨架收窄为该类型涉及的事实路径，只补齐并输出该类型，只有其它类型事实的对象不再出现（口径见 DESIGN.md 同日修正）。

技能明细以各指标实际出现的“维度对象 + 技能”组合为骨架，每个组合补齐三类任务。纯技能目录中未出现活动的技能、没有技能活动的经理不会展开成零矩阵。指定 `task_type` 时同样只取该类型的“维度对象+技能”组合与该类型行。筛选结果没有关联统计技能时返回 items=[]，提示 `no_matching_skills`。

未绑定统计技能的任务不进推送口径（汇总与技能明细一致），因此不生成虚构的“未知技能”行。技能明细与汇总的差异只剩“多技能任务重复归属、明细不可相加”。
