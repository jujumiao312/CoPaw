...
# 任务类型报表落盘表（供 /api/monitor/cron/report/task-type* 读取；
# 表结构以现场提供的 DDL 为准，数据写入由现场作业负责）。
#
...
# 因此用 dim_hash（组合、任务类型 + 四个维度列的 MD5）承担唯一性，重跑仍可按它覆盖。
#
# 主键不要自增列：报表行的业务唯一键是 (dw_dat_dt, source_id, dim_hash)，直接用它做主键，
# 分布式 TDSQL 的分片键（dw_dat_dt / source_id）也落在主键里；接口不引用自增列。
...
CREATE TABLE IF NOT EXISTS `swe_rm_claw_list_ind_stat` (
                                                           `dw_dat_dt` DATE NOT NULL COMMENT '数据日期(高斯 DW_Dat_Dt)，等于接口 end_date 与统计截止日',
    ...
    `frs_bbk_org_id` VARCHAR(200) NOT NULL DEFAULT '' COMMENT '一级分行号(FRS_BBK_ORG_ID)，不适用为 ALL、名单缺机构号为空串',
    `frs_bbk_org_nm` VARCHAR(500) DEFAULT '' COMMENT '一级分行名称(FRS_BBK_ORG_NM)',
    `brn_org_id` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '网点号(BRN_ORG_ID)，不适用为 ALL',
    `brn_org_nm` VARCHAR(500) DEFAULT '' COMMENT '网点名称(BRN_ORG_NM)',
    `cm_id` VARCHAR(200) NOT NULL DEFAULT '' COMMENT '客户经理编号(CM_ID)，不适用为 ALL',
    `cm_nm` VARCHAR(500) DEFAULT '' COMMENT '客户经理姓名(CM_NM)',
    `pst_lvl` VARCHAR(100) DEFAULT '' COMMENT '岗位定级(PST_LVL)',
    `skill_id` VARCHAR(500) NOT NULL DEFAULT '' COMMENT '技能ID(SKILL_ID)，非技能明细为 ALL',
    `skill_nm` VARCHAR(1000) DEFAULT '' COMMENT '技能中文名(SKILL_NM)',
    `job_type` VARCHAR(32) NOT NULL COMMENT '任务类型(高斯 JOB_TYPE): push_plan/ask_plan/push_other',
    `job_type_nm` VARCHAR(100) DEFAULT '' COMMENT '任务类型名称(JOB_TYPE_NM)',
    ...
    `vld_ctc_cust_qty` BIGINT DEFAULT NULL COMMENT '强接触客户数',
    `vld_ctc_cust_rate` DECIMAL(38,9) DEFAULT NULL COMMENT '强接触客户率',
    `wlth_prod_buy_cm_qty_t1` BIGINT DEFAULT NULL COMMENT '财富产品购买客户经理人数(T+1)',
    `wlth_prod_buy_cust_qty_t1` BIGINT DEFAULT NULL COMMENT '财富产品购买客户数(T+1)',
    `wlth_prod_buy_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '财富产品购买金额(T+1)',
    `aum_asc_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT 'AUM提升金额(T+1)',
    `sfl_cust_asc_qty_t1` BIGINT DEFAULT NULL COMMENT '金葵花客户提升数(T+1)',
    `wlth_inc_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '财富中收(T+1)',
    `fnd_prod_buy_cm_qty_t1` BIGINT DEFAULT NULL COMMENT '基金购买客户经理人数(T+1)',
    `dps_prod_buy_cm_qty_t1` BIGINT DEFAULT NULL COMMENT '存款购买客户经理人数(T+1)',
    `fp_prod_buy_cm_qty_t1` BIGINT DEFAULT NULL COMMENT '理财购买客户经理人数(T+1)',
    `insu_prod_buy_cm_qty_t1` BIGINT DEFAULT NULL COMMENT '保险购买客户经理人数(T+1)',
    `fnd_prod_buy_cust_qty_t1` BIGINT DEFAULT NULL COMMENT '基金购买客户数(T+1)',
    `dps_prod_buy_cust_qty_t1` BIGINT DEFAULT NULL COMMENT '存款购买客户数(T+1)',
    `fp_prod_buy_cust_qty_t1` BIGINT DEFAULT NULL COMMENT '理财购买客户数(T+1)',
    `insu_prod_buy_cust_qty_t1` BIGINT DEFAULT NULL COMMENT '保险购买客户数(T+1)',
    `fnd_prod_buy_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '基金购买金额(T+1)',
    `dps_prod_buy_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '存款购买金额(T+1)',
    `fp_prod_buy_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '理财购买金额(T+1)',
    `insu_prod_buy_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '保险购买金额(T+1)',
    `fnd_aum_asc_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '基金AUM提升金额(T+1)',
    `dps_aum_asc_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '存款AUM提升金额(T+1)',
    `fp_aum_asc_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '理财AUM提升金额(T+1)',
    `insu_aum_asc_amt_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '保险AUM提升金额(T+1)',
    `fnd_inc_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '基金中收(T+1)',
    `dps_inc_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '存款中收(T+1)',
    `fp_inc_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '理财中收(T+1)',
    `insu_inc_t1` DECIMAL(38,9) DEFAULT NULL COMMENT '保险中收(T+1)',
    `wlth_prod_buy_cm_qty_t3` BIGINT DEFAULT NULL COMMENT '财富产品购买客户经理人数(T+3)',
    `wlth_prod_buy_cust_qty_t3` BIGINT DEFAULT NULL COMMENT '财富产品购买客户数(T+3)',
    `wlth_prod_buy_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '财富产品购买金额(T+3)',
    `aum_asc_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT 'AUM提升金额(T+3)',
    `sfl_cust_asc_qty_t3` BIGINT DEFAULT NULL COMMENT '金葵花客户提升数(T+3)',
    `wlth_inc_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '财富中收(T+3)',
    `fnd_prod_buy_cm_qty_t3` BIGINT DEFAULT NULL COMMENT '基金购买客户经理人数(T+3)',
    `dps_prod_buy_cm_qty_t3` BIGINT DEFAULT NULL COMMENT '存款购买客户经理人数(T+3)',
    `fp_prod_buy_cm_qty_t3` BIGINT DEFAULT NULL COMMENT '理财购买客户经理人数(T+3)',
    `insu_prod_buy_cm_qty_t3` BIGINT DEFAULT NULL COMMENT '保险购买客户经理人数(T+3)',
    `fnd_prod_buy_cust_qty_t3` BIGINT DEFAULT NULL COMMENT '基金购买客户数(T+3)',
    `dps_prod_buy_cust_qty_t3` BIGINT DEFAULT NULL COMMENT '存款购买客户数(T+3)',
    `fp_prod_buy_cust_qty_t3` BIGINT DEFAULT NULL COMMENT '理财购买客户数(T+3)',
    `insu_prod_buy_cust_qty_t3` BIGINT DEFAULT NULL COMMENT '保险购买客户数(T+3)',
    `fnd_prod_buy_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '基金购买金额(T+3)',
    `dps_prod_buy_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '存款购买金额(T+3)',
    `fp_prod_buy_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '理财购买金额(T+3)',
    `insu_prod_buy_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '保险购买金额(T+3)',
    `fnd_aum_asc_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '基金AUM提升金额(T+3)',
    `dps_aum_asc_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '存款AUM提升金额(T+3)',
    `fp_aum_asc_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '理财AUM提升金额(T+3)',
    `insu_aum_asc_amt_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '保险AUM提升金额(T+3)',
    `fnd_inc_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '基金中收(T+3)',
    `dps_inc_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '存款中收(T+3)',
    `fp_inc_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '理财中收(T+3)',
    `insu_inc_t3` DECIMAL(38,9) DEFAULT NULL COMMENT '保险中收(T+3)',
    `wlth_prod_buy_cm_qty_t7` BIGINT DEFAULT NULL COMMENT '财富产品购买客户经理人数(T+7)',
    `wlth_prod_buy_cust_qty_t7` BIGINT DEFAULT NULL COMMENT '财富产品购买客户数(T+7)',
    `wlth_prod_buy_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '财富产品购买金额(T+7)',
    `aum_asc_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT 'AUM提升金额(T+7)',
    `sfl_cust_asc_qty_t7` BIGINT DEFAULT NULL COMMENT '金葵花客户提升数(T+7)',
    `wlth_inc_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '财富中收(T+7)',
    `fnd_prod_buy_cm_qty_t7` BIGINT DEFAULT NULL COMMENT '基金购买客户经理人数(T+7)',
    `dps_prod_buy_cm_qty_t7` BIGINT DEFAULT NULL COMMENT '存款购买客户经理人数(T+7)',
    `fp_prod_buy_cm_qty_t7` BIGINT DEFAULT NULL COMMENT '理财购买客户经理人数(T+7)',
    `insu_prod_buy_cm_qty_t7` BIGINT DEFAULT NULL COMMENT '保险购买客户经理人数(T+7)',
    `fnd_prod_buy_cust_qty_t7` BIGINT DEFAULT NULL COMMENT '基金购买客户数(T+7)',
    `dps_prod_buy_cust_qty_t7` BIGINT DEFAULT NULL COMMENT '存款购买客户数(T+7)',
    `fp_prod_buy_cust_qty_t7` BIGINT DEFAULT NULL COMMENT '理财购买客户数(T+7)',
    `insu_prod_buy_cust_qty_t7` BIGINT DEFAULT NULL COMMENT '保险购买客户数(T+7)',
    `fnd_prod_buy_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '基金购买金额(T+7)',
    `dps_prod_buy_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '存款购买金额(T+7)',
    `fp_prod_buy_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '理财购买金额(T+7)',
    `insu_prod_buy_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '保险购买金额(T+7)',
    `fnd_aum_asc_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '基金AUM提升金额(T+7)',
    `dps_aum_asc_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '存款AUM提升金额(T+7)',
    `fp_aum_asc_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '理财AUM提升金额(T+7)',
    `insu_aum_asc_amt_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '保险AUM提升金额(T+7)',
    `fnd_inc_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '基金中收(T+7)',
    `dps_inc_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '存款中收(T+7)',
    `fp_inc_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '理财中收(T+7)',
    `insu_inc_t7` DECIMAL(38,9) DEFAULT NULL COMMENT '保险中收(T+7)',
    `wlth_prod_buy_cm_qty_t14` BIGINT DEFAULT NULL COMMENT '财富产品购买客户经理人数(T+14)',
    `wlth_prod_buy_cust_qty_t14` BIGINT DEFAULT NULL COMMENT '财富产品购买客户数(T+14)',
    `wlth_prod_buy_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '财富产品购买金额(T+14)',
    `aum_asc_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT 'AUM提升金额(T+14)',
    `sfl_cust_asc_qty_t14` BIGINT DEFAULT NULL COMMENT '金葵花客户提升数(T+14)',
    `wlth_inc_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '财富中收(T+14)',
    `fnd_prod_buy_cm_qty_t14` BIGINT DEFAULT NULL COMMENT '基金购买客户经理人数(T+14)',
    `dps_prod_buy_cm_qty_t14` BIGINT DEFAULT NULL COMMENT '存款购买客户经理人数(T+14)',
    `fp_prod_buy_cm_qty_t14` BIGINT DEFAULT NULL COMMENT '理财购买客户经理人数(T+14)',
    `insu_prod_buy_cm_qty_t14` BIGINT DEFAULT NULL COMMENT '保险购买客户经理人数(T+14)',
    `fnd_prod_buy_cust_qty_t14` BIGINT DEFAULT NULL COMMENT '基金购买客户数(T+14)',
    `dps_prod_buy_cust_qty_t14` BIGINT DEFAULT NULL COMMENT '存款购买客户数(T+14)',
    `fp_prod_buy_cust_qty_t14` BIGINT DEFAULT NULL COMMENT '理财购买客户数(T+14)',
    `insu_prod_buy_cust_qty_t14` BIGINT DEFAULT NULL COMMENT '保险购买客户数(T+14)',
    `fnd_prod_buy_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '基金购买金额(T+14)',
    `dps_prod_buy_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '存款购买金额(T+14)',
    `fp_prod_buy_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '理财购买金额(T+14)',
    `insu_prod_buy_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '保险购买金额(T+14)',
    `fnd_aum_asc_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '基金AUM提升金额(T+14)',
    `dps_aum_asc_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '存款AUM提升金额(T+14)',
    `fp_aum_asc_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '理财AUM提升金额(T+14)',
    `insu_aum_asc_amt_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '保险AUM提升金额(T+14)',
    `fnd_inc_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '基金中收(T+14)',
    `dps_inc_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '存款中收(T+14)',
    `fp_inc_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '理财中收(T+14)',
    `insu_inc_t14` DECIMAL(38,9) DEFAULT NULL COMMENT '保险中收(T+14)',
    `stat_start_dt` DATE DEFAULT NULL COMMENT '统计区间起始日 = dw_dat_dt 当月 1 号（写入时由 dw_dat_dt 推导）',
    `stat_end_dt` DATE DEFAULT NULL COMMENT '统计区间截止日 = dw_dat_dt（写入时由 dw_dat_dt 推导）',
    `loaded_at` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '本次写入时间',
    `dim_hash` CHAR(32) COLLATE ascii_bin AS (md5(concat_ws(char(1), `rpt_combo`, `job_type`, `frs_bbk_org_id`, `brn_org_id`, `cm_id`, `skill_id`))) STORED COMMENT '组合+任务类型+四维度键的 MD5，主键用它规避宽列索引长度上限',
    PRIMARY KEY (`dw_dat_dt`, `source_id`, `dim_hash`),
    KEY `idx_swe_rcli_branch` (`dw_dat_dt`, `source_id`, `rpt_combo`, `frs_bbk_org_id`, `brn_org_id`),
    KEY `idx_swe_rcli_skill` (`dw_dat_dt`, `source_id`, `rpt_combo`, `skill_id`),
    KEY `idx_swe_rcli_user` (`dw_dat_dt`, `source_id`, `rpt_combo`, `cm_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='金葵花RM小助CLAW名单指标统计落盘快照(数据源:高斯，主键dim_hash upsert)';
"""
...
async def init_database_tables() -> None:
    ...
    try:
        ...
        logger.info(
            f"Created {new_table} table (or already exists)",
        )
