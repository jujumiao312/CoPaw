...
--   5. NDS 枚举入口仍用 lower(列) 兼容大小写；进入任务/点击临时表后统一保存为小写，
--      下游规则匹配不再重复 lower()。确认源库枚举大小写固定后，可进一步改为原生等值过滤；
...
select distinct
  ...
  ,lower(J.STATUS) as JOB_STATUS    /* 任务当前状态（统一小写） */
...
select
  ...
  ,lower(C.EVENT_TYPE) as EVENT_TYPE                /* 事件类型（统一小写） */
  ,lower(C.TEMPLATE_TYPE) as TEMPLATE_TYPE          /* 模板类型（统一小写） */
  ,lower(C.BUTTON_TYPE) as BUTTON_TYPE              /* 按钮类型（统一小写） */
