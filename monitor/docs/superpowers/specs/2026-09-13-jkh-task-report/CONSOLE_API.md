...
新增 `task_type_report_export.py` 沿用项目已有 openpyxl，不改旧导出实现。使用 write-only 工作簿，只包含“统计报表”或“技能明细”一个工作表；2026-09-17 起不再生成“筛选与口径”表。

列集合跟随当前导出的报表，不在该报表里的字段不出现：

- 维度列按 `group_by` 选取：`overall` 没有标识列；`branch` 只有分行号和分行名称；`org` 再加网点号和网点名称；`manager` 再加客户经理ID、客户经理姓名、SAP号和SAP岗位。
- 技能名称（`cn_name`）只在 `skill_detail=true` 出现，技能 ID 不作为独立列导出。
- 指标列按该任务类型是否真正产出选取：`ask_plan` 不导出活跃客户经理人数；`push_other` 不导出方案客户数、已查看方案客户数、方案查看率、洞察客户数、洞察覆盖率、点击客户洞察总次数、电访客户数、电访覆盖率、点击去电访总次数。其余 null 为空，0 为数值0。
